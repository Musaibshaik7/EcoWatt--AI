...
#     horizon_days = st.slider("Forecast horizon (days)", 7, 30, 14, key="horizon_days")
...
# sel_left, sel_mid, sel_right = st.columns([1, 2, 1])
# with sel_mid:
...
#     }).sort_values("date").tail(st.session_state.horizon_days).reset_index(drop=True)
...
#     df["solar_scale"] = (df["solar_kwh_m2"]/avg_solar).fillna(1.0)
#     df["wind_scale"] = (df["wind_m_s_avg"]/avg_wind).fillna(1.0)
...
#     wind_score = min((avg_wind/10)*100,100)
...
#                     title = name
#                     if name=="Solar":
#                         y_cols=["solar_mj_m2"]; title="☀️ Daily Solar (MJ/m²)"
#                     elif name=="Wind":
#                         y_cols=["wind_m_s_avg"]; title="🌬️ Wind Speed (m/s)"
